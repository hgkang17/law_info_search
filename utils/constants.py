...
# 본문 전체가 함께 바뀐다. 본문 위 글꼴 칸에서 사용자가 다른 글꼴을 고를
# 수 있고, 고른 값은 설정에 남는다.
...
# 조문 팝업ㆍ3단비교 팝업을 처음 열 때의 글자 크기(pt). 본문ㆍ별표와 같은
# 굴림 9.5pt로 맞춘다. 예전에는 좁은 팝업에 맞춘다며 0.5pt 작게 열었는데,
# 본문에서 링크를 눌러 팝업이 뜰 때마다 글자 크기가 바뀌어 같은 조문이
# 다른 문서처럼 보였다. 팝업 머리줄의 가+ㆍ가- 단추로 조절한 값은 설정에
# 남아 모든 팝업에 함께 적용된다. 줄 간격은 배수(1.35)라서 글자 크기를
# 바꾸면 저절로 따라 넓어진다.
DEFAULT_POPUP_FONT_POINT = 9.5
# 팝업 기본 크기를 본문과 같은 9.5로 올리면서, 예전 기본값 9.0이 그대로
# 저장돼 있는 설정만 한 번 옮긴다. 사용자가 일부러 고른 다른 크기는 두고,
# 옮긴 뒤 다시 9.0으로 고른 값도 다음 실행에서 덮지 않는다.
POPUP_FONT_DEFAULTS_VERSION = 1
LEGACY_POPUP_FONT_POINT = 9.0
